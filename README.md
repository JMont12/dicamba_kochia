...
gatk VariantFiltration -V combined_SNPs.vcf -filter "QD < 5.0" --filter-name "QD5" -filter "QUAL < 30.0" --filter-name "QUAL30" -filter "SOR > 2.0" --filter-name "SOR2" -filter "FS > 10.0" --filter-name "FS10" -filter "MQ < 0.0" --filter-name "MQ50" -O snps_filtered.vcf
